...
function t_ffmpeg_bitrate {
	...
	ffmpeg -y -i "$MP4_FNAME_FULL" -c:v libx264 -s "$MP4_WIDTH":"$MP4_HEIGHT" -preset slow -b:v "$BITRATE"k -pass 1 -movflags +faststart -profile:v baseline -level 4.0 -c:a libfdk_aac -b:a 128k -f mp4 /dev/null && ffmpeg -y -i "$MP4_FNAME_FULL" -c:v libx264 -s "$MP4_WIDTH":"$MP4_HEIGHT" -preset slow -b:v "$BITRATE"k -pass 2 -movflags +faststart -profile:v baseline -level 4.0 -c:a libfdk_aac -b:a 128k "$MP4_OUT"
#
}
...
function t_html5_video_suite {
	...
	OUT_FILE="$ORIG_MP4_FNAME"".mpd"

	MP4Box -dash 2000 -rap -frag-rap -profile onDemand -out "$OUT_FILE" "$MP4_OUT_Q1" "$MP4_OUT_Q2" "$MP4_OUT_Q3" "$MP4_OUT_Q4"

	# HLS segmented file
	ffmpeg -y -i "$MP4_OUT_Q3" -map 0 -codec:v libx264 -codec:a libfdk_aac -f ssegment -segment_list "$ORIG_MP4_FNAME".m3u8 -segment_list_flags +live -segment_time 10 "$ORIG_MP4_FNAME"%03d.ts
